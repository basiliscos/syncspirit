...
struct SYNCSPIRIT_API global_discovery_actor_t final : public model_actor_t<r::actor_base_t>,
    ...
    template <typename Actor> struct config_builder_t : parent_t::template config_builder_t<Actor> {
        ...
        builder_t &&rx_buff_size(const std::uint32_t value) && noexcept {
            base_t::config.rx_buff_size = value;
            return std::move(*static_cast<typename base_t::builder_t *>(this));
        }

        builder_t &&io_timeout(const std::uint32_t value) && noexcept {
            base_t::config.io_timeout = value;
            return std::move(*static_cast<typename base_t::builder_t *>(this));
        }
