...
int folder_context_t::process(unexamined_t &child_info, stack_context_t &ctx) noexcept {
    ...
    if (type == proto::FileInfoType::DIRECTORY) {
        ...
        if (recurse) {
            stack.push_front(
                unscanned_dir_t(std::move(child_info), child_info.recurse_children, child_info.requires_refinement));
        }
    ...
    for (auto it_disk = infos.begin(); it_disk != infos.end(); ++it_disk) {
        ...
        if (info.ec) {
            ...
            if (fs::is_temporal(info.path)) {
                ...
                auto refine = task.requires_refinement;
                auto task = unexamined_t(std::move(child), recurse, recurse, refine);
