...
struct folder_context_t : boost::intrusive_ref_counter<folder_context_t, boost::thread_safe_counter> {
  ...
  private:
    ...
    bool ensure_folder_existance(stack_context_t &ctx) noexcept;
...
using unexamined_items_t = std::vector<stack_item_t>;

folder_context_ptr_t make_context(model::folder_info_ptr_t local_folder, std::string_view start_subdir,
