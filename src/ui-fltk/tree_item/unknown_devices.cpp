...
void unknown_devices_t::build_tree() {
    ...
    tree()->begin();
    for (auto it : devices) {
        auto label = it.item->get_name().data();
        auto node = new tree_item_t(supervisor, tree());
        node->label(label);
        add(prefs(), label, node);
    }
    tree()->end();
}
...
auto unknown_devices_t::operator()(const diff::load::load_cluster_t &diff, void *data) noexcept -> outcome::result<void> {
    return diff.diff::cluster_aggregate_diff_t::visit(*this, data);
}

auto unknown_devices_t::operator()(const diff::load::unknown_devices_t &diff, void *) noexcept -> outcome::result<void> {
    build_tree();
    update_label();
