...
local_win_t::local_win_t(const cluster_t &cluster, sequencer_t &sequencer, proto::FileInfo proto_file_,
    ...
    : advance_t(folder_id_, peer_id_, advance_action_t::resolve_local_win) {
    ...
    auto remote_file = remote_files.by_name(proto_file_.name());
    assert(remote_file);
    initialize(cluster, sequencer, std::move(proto_file_), remote_file->make_conflicting_name());
}
