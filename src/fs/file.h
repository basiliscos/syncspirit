...
struct SYNCSPIRIT_API file_t : model::arc_base_t<file_t> {
    ...
    static outcome::result<file_t> open_write(model::file_info_ptr_t model, const model::folder_info_t& folder_info) noexcept;
    static outcome::result<file_t> open_read(const bfs::path &path) noexcept;

  private:
    using backend_ptr_t = std::unique_ptr<utils::fstream_t>;
    file_t(utils::fstream_t backend, model::file_info_ptr_t model, bfs::path path, bfs::path model_path, bool temporal) noexcept;
