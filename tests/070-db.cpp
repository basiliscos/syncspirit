...
TEST_CASE("save & load folder", "[db]") {
    ...
    auto d1 = model::device_id_t::from_string("KHQNO2S-5QSILRK-YX4JZZ4-7L77APM-QNVGZJT-EKU7IFI-PNEPBMY-4MXFMQD").value();
    auto d2 = model::device_id_t::from_string("KUEQE66-JJ7P6AD-BEHD4ZW-GPBNW6Q-Y4C3K4Y-X44WJWZ-DVPIDXS-UDRJMA7").value();
    ...
    *folder_src.add_devices() = device_src_1;
    ...
    CHECK(fd1.device == device2);
    CHECK(fd1.index_id == 123);
    CHECK(fd1.max_sequence == 0);

    auto& fd2 = *(++folder->devices.begin());
    CHECK(fd2.device == device1);
    CHECK(fd2.index_id == 5);
    CHECK(fd2.max_sequence == 0);
}
