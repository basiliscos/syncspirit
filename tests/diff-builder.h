...
struct SYNCSPIRIT_TEST_API diff_builder_t : private model::diff::apply_controller_t {
    ...
    diff_builder_t &append_block(const model::file_info_t &target, const model::folder_info_t& target_fi, size_t block_index, utils::bytes_t data) noexcept;
    diff_builder_t &clone_block(const model::file_block_t &, const model::folder_info_t& target_fi, const model::folder_info_t& source_fi) noexcept;
    ...
    diff_builder_t &mark_reacheable(model::file_info_ptr_t peer_file, const model::folder_info_t& peer_fi, bool value) noexcept;
