...
SYNCSPIRIT_TEST_API model::path_t locate_path(const char *test_file);
SYNCSPIRIT_TEST_API std::string read_file(const bfs::path &path);
SYNCSPIRIT_TEST_API std::string read_file(const model::poly_path_view_t &path);
SYNCSPIRIT_TEST_API std::string read_file(const model::path_t &path);
